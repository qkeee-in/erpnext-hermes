...
def ensure_qkeee_env_file_skeleton() -> bool:
    ...
    generated new ones via ensure_bot_user.py — this skill's SKILL.md is
    explicit that credentials only ever land in this file via the
    operator's own manual copy-paste from a one-time stdout print, never
    by this tooling reading them back and re-writing them. This function
    exists only so a user running qkeee-erp-bot-init on a truly fresh
    ...
    if os.path.exists(path):
        return False
    header = (
        "# qkeee-erp.env — ERPNext credentials for the qkeee-erp-* skill library.\n"
        "# Created by qkeee-erp-bot-init. Add three lines per environment tag:\n"

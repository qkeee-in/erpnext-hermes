...
def get_resource(tag: str, doctype: str, name: str, strip_noise: bool = True,
    ...
    needs checking (e.g. Asset finance-book/depreciation-schedule review
    ...
    `modified` is deliberately kept (unlike other qkeee-erp-* copies) so
    a caller can pass it straight through as mutate_resource()'s
    expected_modified for the submit-time TOCTOU check.
...
def mutate_resource(tag: str, doctype: str, action: str, payload: dict = None,
    ...
    this change) is required for every write — the connector
    authenticates as a shared bot account, so without this the ERPNext
    audit trail would show only the bot. On success, a best-effort
    ...
    `modified` timestamp from when it last read/staged the draft, pass
    it here. Before submitting, the freshly re-fetched doc's `modified`
    is compared against it; a mismatch means someone else edited the
    record between staging and submit, and this raises rather than
    silently submitting a doc the user never actually reviewed. This
    narrows (does not eliminate) the fetch-then-submit TOCTOU gap — the
    remaining unmitigated window is between this check and the submit
    POST itself, which frappe.client.submit's own TimestampMismatchError
    is the final backstop for.
    ...
    try:
        result = _do_mutate(cfg, doctype, action, payload, name, requested_by, expected_modified)
    except ConnectorError as e:
...
def _do_mutate(cfg: dict, doctype: str, action: str, payload: dict, name: str, requested_by: str,
               expected_modified: str = None) -> dict:
    if action == "create":
        path = f"/api/resource/{urllib.parse.quote(doctype)}"
    ...
    if action == "submit":
        ...
        if expected_modified is not None and full_doc.get("modified") != expected_modified:
            raise ConnectorError(
                f"'{doctype}' '{name}' was modified since it was last staged "
                f"(expected modified={expected_modified!r}, now {full_doc.get('modified')!r}). "
        ...
        if args.command == "health":
            ...
            print(json.dumps(
                mutate_resource(args.tag, args.doctype, args.action, payload, args.name,
                                 args.mode, args.expected_modified, args.requested_by,

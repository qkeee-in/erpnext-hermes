...
Audit Log's `session` field is a plain string correlator, populated via
`--session-id`/`session_id`, with no doctype of its own behind it. All
of this is best-effort — see "Audit logging is best-effort, not a gate"
below for why, and
...
# PERSONA_DOCTYPE is deliberately NOT here (removed 2026-08-23) — persona
# registration is single-digit-row, create-only, non-recursive, and is
# now audited like any other write; see ensure_persona_registered().

#!/usr/bin/env python3
"""
qkeee-erp-catch-all discovery helper — resolves what's actually installed
on a target ERPNext instance (apps + versions, DocType-to-module-to-app
mapping, live field schema) so the catch-all skill can reason from real
metadata instead of guessing.

Self-contained: stdlib only, imports get_env_config()/_request() from the
sibling erp_client.py copy (same connector every other qkeee-erp-* skill
...
def list_installed_apps(tag: str, *, debug: bool = False, requested_by: str = None,
    ...
    REST read, confirmed working) is the expected next step — not a rare
    fallback, see SKILL.md/domain-knowledge.md step ordering.
    """
...
def doctype_meta(tag: str, doctype: str, *, debug: bool = False, requested_by: str = None,
    ...
    can 403 under a correctly least-privileged shared bot account (see
    the module plan's bot-init least-privilege decision). A caller
    hitting that should surface it as a permissions gap to fix, not
    treat it as "doctype doesn't exist."
    """
    cfg = get_env_config(tag)
...
def resolve_doctype(tag: str, doctype: str, *, debug: bool = False, requested_by: str = None,
    ...
    a caller (and this skill's non-negotiable: never state something
    isn't confirmed as if it were) can't conflate them: a doctype with no
    `module` at all reports `app: null, app_lookup_error: null` (nothing
    to look up — genuinely no module recorded); a doctype whose Module
    Def lookup itself failed (permission denied, network error, a module
    name that doesn't resolve) reports `app: null, app_lookup_error:
    "<the ConnectorError message>"` — a fetch that failed, not a
    confirmed absence. Before this fix, both cases collapsed to a bare
    `app: null`, risking a false "this is custom, no owning app" claim
    when the lookup had actually just errored.
    """
...
def _cli():
    p = argparse.ArgumentParser(description="qkeee-erp-catch-all discovery CLI")
    ...
    p.add_argument("--persona-code", default="qkeee-erp-catch-all",
                   help="threaded into audit rows (default: qkeee-erp-catch-all)")

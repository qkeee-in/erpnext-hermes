#!/usr/bin/env python3
"""
qkeee-erp-accounts-executive connector — read+write copy of the canonical
...
by qkeee_erp.mode), unlike qkeee-erp-mis-analyst's read-only-only copy.
...
skill's prompt. A second, capability-specific gate (advisory-first for
Journal Entry submission) lives in scripts/render_je_draft.py, not here
— this file enforces the library-wide mode gate; the JE-specific
never-auto-submit rule is enforced closer to where JE drafts are built.
...
SKILL_LABEL = "qkeee-erp-accounts-executive"
...
def _request(cfg: dict, method: str, path: str, params: dict = None, payload: dict = None) -> dict:
    ...
    # an explicit UA.
    req.add_header("User-Agent", "qkeee-erp-accounts-executive/1.0")
...
# Sales Order doc: ~38% byte reduction (same field shapes recur across
# ERPNext doctypes).
_NOISE_FIELDS = {
...
def get_resource(tag: str, doctype: str, name: str, strip_noise: bool = True,
    ...
    needs checking (e.g. Journal Entry line-item review before submit) —
...
def run_query_report(tag: str, report_name: str, filters: dict = None,
                      *, debug: bool = False, session_id: str = None, persona_code: str = None,
                      requested_by: str = None) -> dict:
    """Run one of ERPNext's own built-in reports server-side (e.g.
    "Accounts Receivable", "Accounts Payable", "Sales Register", "Purchase
    Register") instead of hand-aggregating raw GL/invoice rows into the
    same shape — see references/erpnext-accounting-docs.md for the
    report_name -> capability map. Read-only in effect (runs a report,
    creates nothing) despite being called over POST.

    `debug=True` logs this read to Qkeee Bot Audit Log, same as
    query_resource()/get_resource() — logged against reference_doctype
    "Report" with reference_name=report_name, since a query report isn't
    itself a DocType record being read.
    """
    cfg = get_env_config(tag)
...
def mutate_resource(tag: str, doctype: str, action: str, payload: dict = None,
    ...
    skill's capability-specific advisory-first rules (e.g. Journal Entry
    submission always needs a separate explicit confirm regardless of
    mode) — those live in the calling skill logic / render_je_draft.py,
    not here.
...
def _cli():
    ...
    if args.command in ("health", "query", "get", "report", "mutate",

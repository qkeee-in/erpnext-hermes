...
def _request(cfg: dict, method: str, path: str, params: dict = None, payload: dict = None) -> dict:
    ...
    # an explicit UA.
    req.add_header("User-Agent", "qkeee-erp-hr-associate/1.0")
...
def health_check(tag: str = "default") -> dict:
    ...
    permission on any specific DocType (e.g. an HR-restricted role may
    still get a 403 on Employee later). Report that as its own distinct
    failure mode, not folded into a connectivity failure.
    """
...
def query_resource(tag: str, doctype: str, filters: list = None, fields: list = None, limit: int = 20,
    ...
    a read-heavy persona can generate far more Read calls than any other
    action type, so logging every read unconditionally would have made
    Audit Log itself the volume/bloat problem the debug gate exists to
    avoid. See bot-doctypes-design.md decision 10.
    """
    cfg = get_env_config(tag)
...
def get_resource(tag: str, doctype: str, name: str, strip_noise: bool = True,
    ...
    needs checking — for status/report reads that don't need child
...
def mutate_resource(tag: str, doctype: str, action: str, payload: dict = None,
    ...
    skill's capability-specific advisory-first rule (Offer Letter /
    Employee Onboarding never auto-commit, regardless of mode) — that
    lives in scripts/render_advisory_draft.py and this skill's
    process, not here. Calling mutate_resource() directly for a Job
    Offer or Employee Onboarding create is a bug in the calling skill
    logic, not something this shared gate can prevent by itself.
    ...
    actually ran with the user first — logged to Qkeee Bot Audit Log's
    `user_approved` field for later scanning, not enforced as a gate here
    (see record_audit_log_start()'s docstring and bot-doctypes-design.md
    decision 14). Defaults to False deliberately: a caller that forgets
    to pass it shows up as "Not Confirmed" on scan, which is the intended
...
def _do_mutate(cfg: dict, doctype: str, action: str, payload: dict, name: str, requested_by: str) -> dict:
    ...
    if action == "submit":
        ...
        # caller didn't set. The PII-scope discipline in SKILL.md governs
        # what this skill *writes new values to* via create/update, not

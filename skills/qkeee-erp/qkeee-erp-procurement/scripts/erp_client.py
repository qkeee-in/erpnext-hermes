...
def _cli():
    ...
    if args.command in ("query", "get", "mutate") and not args.session_id:
        ...
        # logging, so a --debug query and a mutate in the same shell
        # session share the visible-to-the-caller id shape consistently.
    ...
    if (args.command in ("query", "get", "report", "mutate") and _is_prod_tag(args.tag)

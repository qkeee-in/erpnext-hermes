#!/usr/bin/env python3
"""
qkeee-erp-procurement connector — read+write copy of the canonical
...
by qkeee_erp.mode): Supplier onboarding, Purchase Order creation, RFQ/
Supplier Quotation writes.
...
skill's prompt. Two further, capability-specific gates live outside
this file, closer to where each draft is built: KYC-completeness for
Supplier onboarding (scripts/render_supplier_draft.py) and the
...
SKILL_LABEL = "qkeee-erp-procurement"
...
def _request(cfg: dict, method: str, path: str, params: dict = None, payload: dict = None) -> dict:
    ...
    req.add_header("User-Agent", "qkeee-erp-procurement/1.0")
...
def query_resource(tag: str, doctype: str, filters: list = None, fields: list = None, limit: int = 20,
    ...
    (best-effort) — see qkeee-erp-core's connector-reference.md for why
    read logging is debug-gated rather than unconditional.
    """
    cfg = get_env_config(tag)
...
def get_user_roles(tag: str, user: str = "") -> dict:
    """Fetch a user's assigned roles — used as the (heuristic, not
    ...
    target = user
    if not target:
        # get_env_config() has no notion of "which user this API key belongs
        # to" (Frappe token auth doesn't expose that directly) — resolve the
        # authenticated user's own username via the health-check endpoint
        # instead.
    ...
    # genuinely holds no procurement role" or a lookup that silently came
    # back thin (wrong username resolved, a permission restriction on the
    # User doctype for this API key, etc). Surface that ambiguity explicitly
    # rather than letting the caller treat empty the same as "checked, no
    # authority" — either way the hard default stays draft-only, but the
    # user deserves to know which case they're in.
    warning = (
        "No roles returned for this user — could mean the user genuinely "
        "holds no procurement role, or that the lookup didn't resolve "
...
def get_resource(tag: str, doctype: str, name: str, strip_noise: bool = True,
    ...
    needs checking (e.g. Purchase Order line-item review before submit) —
    for status/report reads that don't need child tables, use
    query_resource() with --filters + --fields instead, it's ~25x cheaper.
...
def _audit_insert(cfg: dict, fields: dict) -> str:
    try:
        ...
        # (including test-mock exhaustion / unexpected response shapes)
        # that could be mistaken for the real write failing. Still warn to
        # stderr so it's visible in logs rather than a silently empty
        # Audit Log table.
...
def mutate_resource(tag: str, doctype: str, action: str, payload: dict = None,
    ...
    skill's capability-specific gates (KYC-completeness for Supplier,
    submission-authority default for Purchase Order) — those live in
    render_supplier_draft.py / render_po_draft.py, not here.
...
def _do_mutate(cfg: dict, doctype: str, action: str, payload: dict, name: str, requested_by: str) -> dict:
    ...
    if action == "submit":
        ...
        # fields the caller didn't set. A skill's own data-scope discipline
        # governs what it *writes new values to* via create/update, not
        # what a submit call necessarily echoes back to lock the doc.
        #
        ...
        # close this race. Low-probability for a single-operator draft PO,
        # but real for orgs with concurrent procurement users. No
        # mitigation implemented; flag to the user if a submit fails with
        # an unexpected TimestampMismatchError rather than treating it as a
...
def _cli():
    p = argparse.ArgumentParser(description="qkeee-erp-procurement connector CLI")
    p.add_argument("--tag", help="environment tag, from qkeee_erp.active_env (required for health/query/roles/mutate)")
    ...
    q.add_argument("--filters", help="JSON list, e.g. '[[\"status\",\"=\",\"To Bill\"]]'")
    q.add_argument("--fields", help="JSON list, e.g. '[\"name\",\"status\"]'")
    q.add_argument("--limit", type=int, default=20)

    ro = sub.add_parser("roles", help="Fetch a user's assigned roles (PO-authority heuristic)")
    ro.add_argument("--user", default="", help="defaults to the authenticated API user")
    ...
    rp.add_argument("--persona-code", required=True, help="e.g. qkeee-erp-procurement")
    rp.add_argument("--persona-label", required=True, help="display name, e.g. 'Procurement'")
    ...
    os_.add_argument("--persona-code", required=True, help="e.g. qkeee-erp-procurement")
    ...
    lm.add_argument("--related-capability", help="e.g. 'Purchase Order drafting'")
    ...
    if args.command in ("health", "query", "roles", "get", "mutate",
